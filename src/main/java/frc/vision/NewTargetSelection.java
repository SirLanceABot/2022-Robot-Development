...
public class NewTargetSelection implements Runnable {
  ...
  NewGripPipeline newGripPipeline = new NewGripPipeline();

  TargetDataB nextTargetData = new TargetDataB();

  NewImage newImage;

  public NewTargetSelection(NewImage newImage)
  {
...
public void run()
    ...
    while (!Thread.interrupted())
    {
      ...
      this.newImage.getImage(mat);


/* demo GRIP pipeline */
      newGripPipeline.process(mat); // filter the image with the GRIP pipeline
      ...
      filteredContours = new ArrayList<MatOfPoint>(newGripPipeline.filterContoursOutput());
